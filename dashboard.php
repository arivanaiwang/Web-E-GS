...
try {
    $stmt = $pdo->query("SELECT COUNT(*) AS jumlah_pengguna FROM users");
...
try {
    $stmt = $pdo->query("SELECT COUNT(*) AS jumlah_hasil FROM hasil_penangkapan");
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $jumlahHasil = $result['jumlah_hasil'];
} catch (PDOException $e) {
    echo "Query gagal: " . $e->getMessage();
}

try {
    $stmt = $pdo->query("SELECT COUNT(*) AS jumlah_rencana FROM rencana_penangkapan");
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $jumlahRencana = $result['jumlah_rencana'];
} catch (PDOException $e) {
    echo "Query gagal: " . $e->getMessage();
}
?>
