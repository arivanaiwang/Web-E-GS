...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['caption'])) {
    ...
    if ($photo['error'] == 0) {
        ...
        try {
            $query = "INSERT INTO posts (user_id, photo, caption, created_at) VALUES (:user_id, :photo, :caption, NOW())";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['comment'], $_POST['post_id'])) {
    ...
    try {
        ...
        die("Terjadi kesalahan saat menambahkan komentar: " . $e->getMessage());
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['like'], $_POST['post_id'])) {
    ...
    try {
        ...
        die("Terjadi kesalahan saat menambahkan like: " . $e->getMessage());
    }
}
...
try {
    ...
    $stmtComments = $pdo->query($commentsQuery);
    $comments = $stmtComments->fetchAll(PDO::FETCH_GROUP);
} catch (PDOException $e) {
    die("Terjadi kesalahan: " . $e->getMessage());
}
?>
